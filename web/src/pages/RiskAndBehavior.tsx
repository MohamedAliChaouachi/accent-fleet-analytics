...
import { AlertOctagon, Layers, ShieldAlert, Sparkles } from "lucide-react";
...
import { Badge, Input, KpiCard, Panel, Skeleton } from "@/components/ui";
...
export function RiskAndBehavior() {
  ...
  return (
    <PageContainer
      title="Risk & behavior"
      description="Fleet-level + per-device 3-month rolling risk with K-Means cluster overlay."
      actions={<Badge variant="ai">ML-derived</Badge>}
    >
...
function Content({ data }: { data: RiskDashboardResponse }) {
  ...
  const orderedCounts = useMemo(() => {
    const map = new Map(
      data.category_counts.map((c) => [c.category, c.device_count]),
    );
  ...
  const crosstab = useMemo(() => {
    ...
    for (const row of data.cluster_risk_crosstab) {
      ...
      bucket[row.risk_category] =
        (bucket[row.risk_category] ?? 0) + row.devices;
  ...
  const criticalCount =
    orderedCounts.find((c) => c.category === "critical")?.device_count ?? 0;
  const highCount =
    orderedCounts.find((c) => c.category === "high")?.device_count ?? 0;

  if (data.devices.length === 0) {
    return (
      <StateMessage tone="warning">
        No risk data for the current filters. Widen the date range or remove
        the tenant filter — the view derives from a 3-month rolling window, so
        the earliest data must be at least 1 month old.
      </StateMessage>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Panel
        ...
        tone="warning"
        description="Devices grouped by their 3-month rolling risk band."
      >
        {orderedCounts.length === 0 ? (
          ...
          <BarChart
            ...
            rowColors={orderedCounts.map(
              (c) => RISK_COLORS[c.category] ?? "#94a3b8",
            )}
      ...
      <Panel
        ...
        description="Each persona is derived from the K-Means model on monthly driving features. Hover a card for the description."
        tone="ai"
      >
        {data.clusters.length === 0 ? (
          <StateMessage>
            ...
            (or wait for the next incremental flow that follows a successful
            training run).
          </StateMessage>
        ) : (
      ...
      <Panel
        ...
        description="Ordered by risk_score across the filter window. Click a row in the drill-down to explore."
        tone="warning"
        flush
      >
        <DataTable
      ...
      {data.fleet.length ? (
        <Panel
          ...
          description="One row per tenant."
          tone="primary"
          flush
        >
          <DataTable
...
function ClusterPersonaGrid({
  ...
  if (sizes.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">No cluster assignments.</p>
    );
  }
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {sizes.map((c, i) => {
        ...
        return (
          <div
            key={c.cluster_id}
            className="group relative overflow-hidden rounded-xl border border-border bg-card p-4 transition hover:-translate-y-0.5 hover:shadow-card"
          >
            <div
              aria-hidden
              className="pointer-events-none absolute inset-0 opacity-50"
              style={{
                background: `linear-gradient(180deg, ${persona.color}1a 0%, transparent 70%)`,
              }}
            />
            <div className="relative flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-2xs uppercase tracking-wider text-muted-foreground">
                  Cluster #{c.cluster_id} · {fmtInt(c.devices)} devices (
                  {pct.toFixed(0)}%)
                </p>
              </div>
            </div>
            <p className="relative mt-3 line-clamp-2 text-xs leading-snug text-muted-foreground">
              {persona.description}
            </p>
            <div className="relative mt-3 h-1.5 w-full rounded-full bg-muted">
              <div
            ...
            <p className="relative mt-2 text-2xs text-muted-foreground">
  ...
  return [
    {
      ...
      header: "Device",
      accessor: (r) => (
        <span className="font-mono text-foreground">#{r.device_id}</span>
      ),
      align: "right",
    },
    {
    ...
    },
    {
      ...
      header: "Behaviour",
      accessor: (r) => (
        <ClusterBadge clusterId={clusterMap.get(r.device_id)?.cluster_id} />
      ),
    },
    {
      key: "trips_3m",
...
function RiskScoreCell({
  ...
  return (
    <div className="flex items-center justify-end gap-2">
      ...
      <span className="w-10 text-right font-semibold num text-foreground">
        {fmtDec(score)}
      </span>
    </div>
  );
...
> = [
  {
    key: "tenant_id",
    header: "Tenant",
    accessor: (r) => r.tenant_label ?? r.tenant_id ?? "—",
  },
  {
    key: "devices_scored",
    header: "Scored",
  ...
  },
  {
    key: "high",
    header: "High",
    accessor: (r) => fmtInt(r.high_devices),
    align: "right",
  },
  {
    key: "moderate",
  ...
  },
  {
    key: "low",
    header: "Low",
    accessor: (r) => fmtInt(r.low_devices),
    align: "right",
  },
  {
    key: "avg_risk_score",
...
function DeviceDrillDown({
  ...
  const selectedCluster =
    deviceId !== null ? clusterMap.get(deviceId) : undefined;
          ...
          {deviceId === null ? (
            ...
            <>
              ...
              {isPending ? (
                ...
                <StateMessage tone="warning">
                  No monthly history for this device.
                </StateMessage>
              ) : (
                <>
                  <div className="rounded-lg border border-border bg-card p-3">
                    ...
                    <LineChart
                      data={
                        data.history as unknown as Array<
                      ...
                      series={[
                        {
                          dataKey: "overspeed_per_100km",
                          label: "Overspeed / 100km",
                        },
                        {
                          dataKey: "night_trip_ratio",
                          label: "Night trip ratio",
                        },
                      ]}
...
function DeviceHeroCard({
  ...
  const drivers = latestHistory
    ? [
        ...
        {
          label: "High-speed trip ratio",
          value: latestHistory.high_speed_trip_ratio,
        },
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-border bg-card p-5 shadow-card">
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${riskColor}1a 0%, ${persona.color}1a 100%)`,
        }}
      />
      ...
      <div className="relative mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-card p-3">
          ...
          <div className="mt-1 flex items-baseline gap-2">
            <span className="text-3xl font-semibold num text-foreground">
          ...
          <div className="mt-2 flex justify-between text-2xs uppercase tracking-wider text-muted-foreground/80">
            <span>low</span>
            <span>moderate</span>
            <span>high</span>
            <span>critical</span>
