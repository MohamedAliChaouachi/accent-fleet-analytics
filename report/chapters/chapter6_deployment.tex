% =====================================================================
%  Chapter 6: Deployment, API, Dashboard and Azure Operations
% =====================================================================
\chapter{Deployment, API, Dashboard and Azure Operations}
\label{chap:deploy}

\section{Introduction}
The evaluation of Chapter~\ref{chap:model} has confirmed the readiness of the behavioural segmentation and of the transparent risk profile for controlled industrialization, while keeping the Isolation Forest anomaly score as a caveated evaluation artefact. The present chapter executes the deployment phase of CRISP-DM. The chapter describes the deployment strategy, the API and the dashboard exposing the analytical outputs, the integration with the analytical PostgreSQL warehouse populated from the converted MySQL dumps of \hostorg, the cron-driven automation, the monitoring and alerting layer, and finally the Microsoft Azure infrastructure on which the platform runs, with a particular focus on the secure remote collaboration setup that allows the two-student team of engineers to operate the platform from separate workstations through SSH tunnelling, IP allowlisting on the Azure Network Security Group, and dedicated service accounts.
...
The VM is associated with an Azure Network Security Group that closes every public port except SSH (port 22) and the HTTPS port of the reverse proxy (port 443). The SSH port is itself restricted to an explicit allowlist of source IP addresses corresponding to the personal workstations of the two-student team and to the office network of \hostorg. This combination ensures that no PostgreSQL port, no API port and no dashboard port is exposed to the public internet, while still allowing both members of the team to connect to the shared analytical environment from their respective home workstations.
...
Three categories of accounts coexist on the VM: \emph{personal accounts}, one per student, used only through SSH key authentication and intended exclusively for human interactive sessions; an \emph{automation account} (\texttt{azureuser}) used by cron and by the systemd services; and a \emph{database service account} (\texttt{accent\_pipeline}) used by the pipeline to authenticate against PostgreSQL, with credentials externalized in an environment file. Each student uses their own SSH key pair, which means that an audit of the system journal can attribute every interactive command to its actual operator without ambiguity.

\section{PostgreSQL integration}

\subsection{Analytical PostgreSQL on the VM}
The analytical PostgreSQL is provisioned in version~16 on the VM, with the three schemas \texttt{staging}, \texttt{warehouse} and \texttt{marts}, plus the operational tables \texttt{etl\_watermark}, \texttt{etl\_run\_log} and \texttt{quarantine\_rejected}. It is populated by the conversion bridge of Chapter~\ref{chap:prep}, which loads the converted MySQL dumps of \hostorg{} into the staging schema, and then by the watermark-driven pipeline described in this chapter. The instance listens only on \texttt{localhost}, which is why remote access from the two students' workstations is mediated by the SSH tunnel described above.
...
\subsection{Telemetry reconstruction for tenant 7486}
Tenant~\texttt{7486} is present in the archive telemetry before it is present in the original trip table. The deployment therefore includes the reconstruction command \path{scripts/reconstruct_telemetry_trips.py}, which rebuilds trip-like records from ignition-on telemetry, recomputes the trip-side and telemetry-side marts, refreshes the materialized risk profile and attempts cluster scoring for the touched months. This step prevents tenant~\texttt{7486} from disappearing from the modeling and dashboard views after a fresh backfill.
...
Model deployment relies on the artefact store described in Chapter~\ref{chap:model}. A new clustering candidate is logged with its scaler, feature order and metrics, then promoted only if the evaluation gate accepts it. The API loads the promoted model from the registry when available, or from the local \texttt{models/clustering/} fallback directory, and can reload the artefact without changing the database contract.
